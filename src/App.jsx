...
const G = () => (
  <style>{`
    ...
    .ng{background:rgba(6,9,15,.9);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border-bottom:1px solid rgba(255,255,255,.06)}
    ...
    .na{color:rgba(255,255,255,.52);transition:color .2s;font-size:14px;font-weight:500;text-decoration:none;background:none;border:none;cursor:pointer;padding:4px 0}
    .na:hover{color:#fff}
    .wc{background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.065);border-radius:20px;transition:transform .35s,border-color .35s;overflow:hidden}
    .wc:hover{transform:translateY(-6px);border-color:rgba(59,130,246,.22)}
    ...
    .ap{background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.07);border-radius:12px;transition:border-color .3s}
    .ap:hover{border-color:rgba(139,92,246,.28)}
...
function Slider({ bfr, aft, stat, slbl }) {
  ...
  useEffect(() => {
    const mm = e => d && mv(e.clientX);
    const mu = () => sd(false);
    const tm = e => d && e.touches[0] && mv(e.touches[0].clientX);
  ...
  return (
    <div ref={r} onMouseDown={e => { sd(true); mv(e.clientX); }} onTouchStart={e => { sd(true); mv(e.touches[0].clientX); }}
      style={{ position: "relative", height: 260, borderRadius: 16, overflow: "hidden", cursor: "col-resize", userSelect: "none" }}>
      <div style={A({ inset: 0 })}>{bfr}</div>
      <div style={A({ inset: 0, clipPath: `inset(0 ${100 - p}% 0 0)` })}>{aft}</div>
      <div style={A({ top: 10, left: 10, fontSize: 9, fontWeight: 700, letterSpacing: ".12em", color: "rgba(255,255,255,.5)", background: "rgba(0,0,0,.62)", padding: "3px 8px", borderRadius: 4, zIndex: 4 })}>BEFORE</div>
      <div style={A({ top: 10, right: 10, fontSize: 9, fontWeight: 700, letterSpacing: ".12em", color: "#fff", background: "#2563eb", padding: "3px 8px", borderRadius: 4, zIndex: 4 })}>AFTER</div>
      {stat && <div style={A({ bottom: 10, right: 10, background: "rgba(0,0,0,.88)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 8, padding: "5px 10px", zIndex: 5 })}>
        <span style={{ color: "#60a5fa", fontWeight: 700, fontSize: 13 }} className="j">{stat}</span>
        <span style={{ color: "rgba(255,255,255,.42)", fontSize: 11, marginLeft: 6 }} className="j">{slbl}</span>
      </div>}
      <div style={A({ top: 0, bottom: 0, left: `${p}%`, transform: "translateX(-50%)", width: 2, background: "rgba(255,255,255,.88)", zIndex: 3 })} />
      <div style={A({ top: "50%", left: `${p}%`, transform: "translate(-50%,-50%)", width: 36, height: 36, borderRadius: "50%", background: "#0f1626", border: "2px solid rgba(255,255,255,.82)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 24px rgba(0,0,0,.7)", zIndex: 4 })}>
        <svg width="14" height="12" viewBox="0 0 14 12" fill="none"><path d="M3.5 6H1M13 6h-2.5M3.5 3L1 6l2.5 3M10.5 3L13 6l-2.5 3" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
      </div>
...
const T = {
  heroBfr: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#111827,#1f2937)", display: "flex", alignItems: "center", justifyContent: "center" }}>
    <div style={{ textAlign: "center", opacity: .38 }}>
      <div style={{ width: 80, height: 52, margin: "0 auto 10px", background: "rgba(255,255,255,.08)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <svg width="20" height="20" viewBox="0 0 20 20" fill="rgba(255,255,255,.4)"><path d="M8 5l8 5-8 5V5z" /></svg>
      </div>
      <div style={{ color: "rgba(255,255,255,.5)", fontSize: 13, fontWeight: 700, letterSpacing: ".04em" }}>10 MONEY TIPS</div>
    </div>
  </div>),
  heroAft: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#0d0d1f,#1a0535)", position: "relative", overflow: "hidden" }}>
    ...
    <div style={{ position: "absolute", left: 12, top: 6, fontSize: 70, fontWeight: 900, color: "#facc15", lineHeight: 1, fontFamily: "system-ui", textShadow: "0 0 32px rgba(250,204,21,.9)", letterSpacing: -2 }}>Y</div>
    <div style={{ position: "absolute", left: 12, bottom: 6, fontSize: 56, fontWeight: 900, color: "#facc15", lineHeight: 1, fontFamily: "system-ui", textShadow: "0 0 32px rgba(250,204,21,.9)" }}>ES</div>
    <div style={{ position: "absolute", left: "41%", bottom: 30, width: 46, height: 46, borderRadius: "50%", border: "2px solid #22d3ee", display: "flex", alignItems: "center", justifyContent: "center", color: "#22d3ee", fontSize: 22, fontWeight: 900, boxShadow: "0 0 22px rgba(34,211,238,.55)" }}>$</div>
    <div style={{ position: "absolute", right: 16, bottom: 22, color: "#ef4444", fontSize: 30, fontWeight: 900, transform: "rotate(-28deg)" }}>↗</div>
  </div>),
  g1bfr: (<div style={{ width: "100%", height: "100%", background: "#111827", display: "flex", alignItems: "center", justifyContent: "center" }}>
  ...
  g1aft: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#050520,#1a0040)", position: "relative", overflow: "hidden" }}>
  ...
  g2bfr: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#8fafc8,#6b8fa5)", display: "flex", alignItems: "center", justifyContent: "center" }}>
    <div style={{ textAlign: "center", color: "rgba(255,255,255,.5)", fontSize: 13 }} className="j">desert landscape<br />before redesign</div>
  </div>),
  g2aft: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#0d1f0d,#1a3a1a)", position: "relative", overflow: "hidden" }}>
  ...
  g3bfr: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#6b7280,#9ca3af)", display: "flex", alignItems: "center", justifyContent: "center" }}>
    <div style={{ textAlign: "center", color: "rgba(255,255,255,.48)", fontSize: 13 }} className="j">generic neutral palette</div>
  </div>),
  g3aft: (<div style={{ width: "100%", height: "100%", position: "relative", overflow: "hidden" }}>
    ...
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ color: "#fff", fontWeight: 900, fontSize: 20, textAlign: "center", textShadow: "0 2px 12px rgba(0,0,0,.7)", lineHeight: 1.3 }} className="s">CONTRAST<br />IS KING</div>
    </div>
  </div>),
  g4bfr: (<div style={{ width: "100%", height: "100%", background: "#1f2937", display: "flex", alignItems: "center", justifyContent: "center" }}>
    <div style={{ color: "rgba(255,255,255,.3)", fontSize: 13, textAlign: "center" }} className="j">standard<br />thumbnail</div>
  </div>),
  g4aft: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#0a0012,#1a0530)", position: "relative", overflow: "hidden" }}>
  ...
  g5bfr: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#bfdbfe,#dbeafe)", display: "flex", alignItems: "center", justifyContent: "center" }}>
    <div style={{ textAlign: "center", color: "rgba(30,58,138,.5)", fontSize: 13 }} className="j">bland channel art<br />50K subs</div>
  </div>),
  g5aft: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#0c1445,#1e3a8a)", position: "relative", overflow: "hidden" }}>
  ...
  g6bfr: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#4ade80,#059669 30%,#1e3a5f 70%,#3b82f6)", position: "relative" }}>
  ...
  g6aft: (<div style={{ width: "100%", height: "100%", background: "linear-gradient(135deg,#1a0040,#2d0060)", position: "relative", overflow: "hidden" }}>
    ...
    <nav className="ng" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100 }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#2563eb,#7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 900, color: "#fff" }} className="s">CC</div>
        ...
        <button className="bp j" onClick={() => st("contact")} style={{ padding: "10px 22px", borderRadius: 10, fontSize: 14, fontWeight: 600 }}>Work With Me</button>
      </div>
    </nav>
  );
}
...
function Hero() {
  ...
  return (
    <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", paddingTop: 68, background: "#06090f", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "64px 24px", width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "center" }}>
        <div>
          ...
          <h1 className="s" style={{ fontSize: 70, fontWeight: 800, lineHeight: 1.02, letterSpacing: "-.03em", marginBottom: 24, ...ani(80) }}>
          ...
          <p className="j" style={{ fontSize: 16, lineHeight: 1.7, color: "rgba(255,255,255,.5)", maxWidth: 440, marginBottom: 36, ...ani(160) }}>
            Designed to <strong style={{ color: "#3b82f6" }}>dominate the feed</strong>. Every pixel engineered for <strong style={{ color: "rgba(255,255,255,.82)" }}>maximum click-through</strong>. Your next video deserves to actually get watched.
          </p>
          <div style={{ display: "flex", gap: 14, marginBottom: 50, ...ani(230) }}>
          ...
          <div style={{ display: "flex", gap: 36, ...ani(300) }}>
            {[{ v: "+40%", l: "Avg. CTR Increase" }, { v: "200+", l: "Thumbnails Delivered" }, { v: "50M+", l: "Views Generated" }].map(s => (
              <div key={s.l}>
                <div className="s" style={{ fontSize: 22, fontWeight: 800, color: "#60a5fa", letterSpacing: "-.02em" }}>{s.v}</div>
                <div className="j" style={{ fontSize: 12, color: "rgba(255,255,255,.38)", marginTop: 3 }}>{s.l}</div>
              </div>
            ))}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 44, ...ani(380) }}>
            <div style={{ width: 1, height: 52, background: "linear-gradient(to bottom,rgba(255,255,255,.32),transparent)" }} />
            <div className="j" style={{ color: "rgba(255,255,255,.2)", fontSize: 11, letterSpacing: ".12em", fontWeight: 500 }}>SCROLL</div>
          </div>
        </div>
        <div style={{ ...ani(200) }}>
          <Slider bfr={T.heroBfr} aft={T.heroAft} stat="+312%" slbl="CTR increase" />
        </div>
      </div>
    </section>
  );
    ...
    <section id="work" style={{ padding: "120px 0", background: "#06090f" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 72 }}>
          ...
          <R delay={160}><p className="j" style={{ color: "rgba(255,255,255,.4)", maxWidth: 520, margin: "24px auto 0", lineHeight: 1.7 }}>Drag the slider on each card to see the transformation. Every redesign is built on research, contrast, and psychology.</p></R>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          {WD.map((c, i) => (
            <R key={i} delay={i * 55}>
              <div className="wc">
                <Slider bfr={c.b} aft={c.a} stat={c.st} slbl={c.sl} />
                <div style={{ padding: "20px 24px 26px" }}>
                  ...
                  <p className="j" style={{ fontSize: 13, color: "rgba(255,255,255,.4)", lineHeight: 1.7, marginBottom: 14 }}>{c.desc}</p>
...
function Process() {
  return (
    <section id="process" style={{ padding: "120px 0", background: "#06090f", position: "relative" }}>
      <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: 700, height: 380, background: "radial-gradient(ellipse,rgba(99,68,255,.065),transparent 70%)", pointerEvents: "none" }} />
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 48, position: "relative", alignItems: "start" }}>
          <div style={{ position: "absolute", top: 68, left: "calc(16.7% + 8px)", right: "calc(16.7% + 8px)", height: 1, background: "linear-gradient(90deg,rgba(139,92,246,.45),rgba(139,92,246,.15) 50%,rgba(139,92,246,.45))", zIndex: 0, pointerEvents: "none" }} />
          {PS.map((s, i) => (
              ...
              <div style={{ textAlign: "center", position: "relative", zIndex: 1 }}>
                <div style={{ display: "flex", justifyContent: "center", marginBottom: 32 }}>
                  <div className="pc" style={{ width: 138, height: 138, borderRadius: "50%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "radial-gradient(circle at 40% 35%,rgba(99,68,255,.14),rgba(99,68,255,.04))" }}>
                    ...
                    <div className="s" style={{ fontSize: 28, fontWeight: 800, color: "rgba(139,92,246,.82)" }}>{s.n}</div>
                  </div>
                </div>
        ...
        <R>
          <div style={{ position: "relative" }}>
            <div style={{ borderRadius: 24, overflow: "hidden", background: "linear-gradient(135deg,#0d1420,#1a0530)", aspectRatio: "4/3", position: "relative" }}>
              ...
              <div style={{ position: "absolute", top: "5%", right: "10%", width: 100, height: "52%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ width: 70, height: 92, background: "radial-gradient(circle at 38% 32%,rgba(50,35,25,.92),rgba(14,10,8,.96))", borderRadius: "50% 50% 44% 44%" }} />
              </div>
            </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          {TM.map((t, i) => (
            <R key={t.h} delay={i * 70}>
              <div className="tc" style={{ padding: "28px 28px" }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px", textAlign: "center" }}>
        ...
        <R delay={80}>
          <div className="cb" style={{ display: "inline-block", padding: "72px 80px", maxWidth: 680, background: "radial-gradient(ellipse at center,rgba(99,68,255,.17) 0%,rgba(37,99,235,.06) 45%,transparent 70%)" }}>
            ...
            <button className="bp j" style={{ padding: "15px 38px", borderRadius: 14, fontSize: 16, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 10 }}>
              Work With Me <span style={{ fontSize: 18 }}>→</span>
            </button>
            <div className="j" style={{ marginTop: 20, fontSize: 13, color: "rgba(255,255,255,.26)" }}>
              No commitments. Email directly at <a href="mailto:workwithme@clickcraft.studio" style={{ color: "#3b82f6", textDecoration: "none" }}>workwithme@clickcraft.studio</a>
            </div>
          </div>
        </R>
      </div>
...
function Footer() {
  return (
    <footer style={{ borderTop: "1px solid rgba(255,255,255,.05)", padding: "32px 24px", textAlign: "center", background: "#06090f" }}>
      ...
      <div className="j" style={{ fontSize: 12, color: "rgba(255,255,255,.16)" }}>© 2024 ClickCraft Studio. All rights reserved.</div>
    </footer>
  );
}
